...
public class UnshippedItemServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (userType == null||password == null||userName == null ) { 
            ...
            response.sendRedirect("login.jsp?message=Please Login As Admin"); 
        }
        ...
        List<OrderPojo> orders=orderDao.getUnshippedOrders();
        for(OrderPojo order:orders){
            String userId=transactionDao.getUserId(order.getOrderId());
            String address=userDao.getUserAddr(userId);
            user_Id.put(order.getOrderId(), userId);
